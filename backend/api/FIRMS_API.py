...
class FIRMS_API_Client:
    ...
    def get_file_data(self, elem_tree=None):
        ...
        document = elem_tree.find(
            self.tag("Document")
        )  # need to step inside the kml tag into the document tag that holds everything
        print(document)
        folders = document.findall(
            self.tag("Folder")
        )  # these are all the folder tags returned into a list
        ...
        for (
            ...
            folders
        ):  # loops through high level overview of folder elements each one contains elements pertaining name, placemark, etc
            ...
            for (
                ...
                elements
            ):  # loop through the elements in the folder element (name, placemark, etc)

                if (
                    e != "Placemark"
                ):  # essentailly all the placemarks are the individual markers/items we want so combine those by folder and contain them in a list so we're handling them in the else statement
                    ...
                    placemarkers = f.findall(
                        self.tag(e)
                    )  # Get all placemarker elements
                    for p in placemarkers:
                        ...
                        for (
                            ...
                            p_elements
                        ):  # loop through the placemarker element and its children ex. description, point, polygon, etc...
                            p_child = p.find(self.tag(ele))
                            match ele:
                                case "description":
                                    desc = {}
                                    desc_text = (
                                        p_child.text.replace("<b>", "")
                                        .replace("</b>", "")
                                        .replace("<br/>", "")
                                        .replace(" ", "")
                                        .replace("\n\n", "")
                                ...
                                case "Polygon":
                                    points = (
                                        p_child.find(self.tag("outerBoundaryIs"))
                                        .find(self.tag("LinearRing"))
                                        .find(self.tag("coordinates"))
                                        .text.replace(" ", "")
                                        .replace("\n\n", "")
                                    )
                        ...
                        placemark_list.append(
                            i_dict
                        )  # append this specific i_dict for one placemarker to the list so we can include all of them under the placemarker key in the main dictionary

            e_dict[
                "Placemark"
            ] = placemark_list  # include list of placemarkers for folder into the main dictionary
            data.append(
                e_dict
            )  # append the main dictionary holding all the information for the specific folder to the data list
        ...
        if response.status_code != 200:
            ...
            for datum in data:
                if "Footprints" in datum["name"]:
                    ...
                    footprint_time_range.append(datum)
            for time_range in footprint_time_range:
                # these are split up by recent hours so this searches through all of them
                placemarks = time_range["Placemark"]
